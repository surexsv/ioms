...
@login_required
@module_required(MODULE_COMPANY_SETTINGS)
def field_operations_settings_view(request):
    if not can_view_company_settings(request.user):
        return access_denied_response(request, reason=REASON_ROLE)

    settings_obj = FieldOperationsSettings.get_solo()
    can_edit = can_manage_company_settings(request.user)

    if request.method == 'POST':
        if not can_edit:
            return access_denied_response(request, reason=REASON_ROLE)
        form = FieldOperationsSettingsForm(request.POST, instance=settings_obj)
        if form.is_valid():
            obj = form.save(commit=False)
            obj.updated_by = request.user
            obj.save()
            messages.success(request, 'Field operations settings updated.')
            return redirect('field_operations_settings')
    else:
        form = FieldOperationsSettingsForm(instance=settings_obj)
        if not can_edit:
            for field in form.fields.values():
                field.disabled = True
